...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <a href="{{ route('found-items.index') }}" class="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 mb-6 transition">
            ...
            <div class="md:w-3/5 p-6 sm:p-8 flex flex-col border-t md:border-t-0 md:border-l border-gray-100">
                <div class="flex items-start justify-between gap-3 mb-5">
                    <div>
                        ...
                        <p class="text-sm text-gray-400 mt-1">Reported by <span class="text-gray-600 font-medium">{{ $item->user->name }}</span></p>
                    </div>
                    <span class="shrink-0 text-xs font-bold px-3 py-1.5 rounded-full uppercase
                        {{ $item->status==='unclaimed' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                        {{ $item->status }}
                    </span>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3 mb-5">
                    ...
                    <div class="bg-gray-50 rounded-xl p-3">
                        ...
                        <p class="font-semibold text-gray-800 text-sm">{{ \Carbon\Carbon::parse($item->date_found)->format('M d, Y') }}</p>
                    </div>
                    <div class="bg-gray-50 rounded-xl p-3">
                        <p class="text-xs text-gray-400 mb-0.5">Time Found</p>
                        <p class="font-semibold text-gray-800 text-sm">{{ \Carbon\Carbon::parse($item->time_found)->format('g:i A') }}</p>
                    </div>
                    <div class="bg-gray-50 rounded-xl p-3 col-span-2">
                        <p class="text-xs text-gray-400 mb-0.5">Place Found</p>
                        <p class="font-semibold text-gray-800 text-sm">{{ $item->place_found }}</p>
                    </div>
                </div>
                    ...
                    <div class="mt-auto space-y-3">
                        ...
                        <form method="POST" action="{{ route('found-items.claim', $item) }}">
                            ...
                            <button type="submit" onclick="return confirm('Mark this item as claimed?')"
                                    class="w-full flex items-center justify-center gap-2 bg-gray-900 hover:bg-gray-700 text-white text-sm font-semibold px-5 py-3 rounded-xl transition">
                                <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
                                Mark as Claimed
                            </button>
                        </form>
