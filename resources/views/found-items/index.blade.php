...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Found Items</h1>
            <p class="text-sm text-gray-500 mt-1">Browse items that have been turned in — one of them might be yours.</p>
        </div>
    ...
    <form method="GET" action="{{ route('found-items.index') }}"
        ...
        <div class="flex gap-2 items-center">
            ...
            @if(request()->hasAny(['search','category','status']))
                <a href="{{ route('found-items.index') }}" class="text-sm text-gray-400 hover:text-gray-600">Clear</a>
            @endif
        </div>
    </form>

    @if(!$items->isEmpty())
        <p class="text-sm text-gray-500 mb-4"><span class="font-semibold text-gray-700">{{ $items->total() }}</span> item{{ $items->total()!==1?'s':'' }} found</p>
    @endif
    ...
    @if($items->isEmpty())
        <div class="bg-white border border-gray-200 rounded-2xl py-20 text-center">
            <div class="bg-blue-50 rounded-2xl flex items-center justify-center mx-auto mb-4" style="width:56px;height:56px">
                <svg width="28" height="28" fill="none" stroke="#3b82f6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><circle cx="11" cy="11" r="6"/><line x1="16.5" y1="16.5" x2="21" y2="21"/></svg>
            </div>
            <p class="font-semibold text-gray-700">No items found</p>
            <p class="text-sm text-gray-400 mt-1">Try different filters or check back later.</p>
        </div>
    @else
                   ...
                   class="group bg-white border border-gray-200 rounded-2xl overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 flex flex-col">
                    <div class="relative bg-gray-50 overflow-hidden" style="height:176px">
                        ...
                        <span class="absolute top-2 right-2 text-xs font-bold px-2.5 py-1 rounded-full
                            {{ $item->status==='unclaimed' ? 'bg-green-500 text-white' : 'bg-gray-400 text-white' }}">
                            {{ ucfirst($item->status) }}
                        </span>
                    </div>
                    <div class="p-4 flex flex-col flex-1 gap-2">
                        ...
                        <div class="mt-auto pt-2 border-t border-gray-100 space-y-1.5 text-xs text-gray-500">
                            <div class="flex items-center gap-1.5">
                                ...
                                <span class="truncate">{{ $item->place_found }}</span>
                            </div>
                            <div class="flex items-center gap-1.5">
